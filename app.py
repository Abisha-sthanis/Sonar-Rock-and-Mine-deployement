from flask import Flask, render_template, request, redirect, url_for
import pickle
import numpy as np

app = Flask(__name__)

# Load trained model
with open('model.pkl', 'rb') as f:
    model = pickle.load(f)

@app.route('/')
def welcome():
    return render_template('welcome.html')

@app.route('/input')
def input_page():
    # Prefilled example
    sample = "0.02,0.0371,0.0428,0.0207,0.0954,0.0986,0.1539,0.1601,0.3109,0.2111,0.1609,0.1582,0.2238,0.0645,0.066,0.2273,0.31,0.2999,0.5078,0.4797,0.5783,0.5071,0.4328,0.555,0.6711,0.6415,0.7104,0.808,0.6791,0.3857,0.1307,0.2604,0.5121,0.7547,0.8537,0.8507,0.6692,0.6097,0.4943,0.2744,0.051,0.2834,0.2825,0.4256,0.2641,0.1386,0.1051,0.1343,0.0383,0.0324,0.0232,0.0027,0.0065,0.0159,0.0072,0.0167,0.018,0.0084,0.009,0.0032"
    return render_template('input.html', sample=sample)

@app.route('/predict', methods=['POST'])
def predict():
    raw = request.form.get('features', '')
    if not raw:
        return redirect(url_for('input_page'))

    try:
        values = [float(x.strip()) for x in raw.split(',') if x.strip() != ""]
    except ValueError:
        return render_template('result.html', error="Invalid input! Please enter numbers only.")

    if len(values) != 60:
        return render_template('result.html', error=f"Expected 60 features, got {len(values)}.")

    arr = np.array(values).reshape(1, -1)
    prediction = model.predict(arr)[0]

    label = "Rock" if prediction == "R" else "Mine"
    return render_template('result.html', prediction=prediction, label=label)

if __name__ == '__main__':
    app.run(debug=True)